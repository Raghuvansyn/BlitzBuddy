    ...
    <div className="relative w-full h-[400px] lg:h-full bg-[radial-gradient(#e5e7eb_1.5px,transparent_1px)] [background-size:20px_20px] bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden flex items-center justify-center font-sans">
      ...
      <div className="relative w-full h-full max-w-2xl max-h-2xl mx-auto">
        {developers.map((dev) => (
          ...
          >
            {/* The Avatar Dot */}
            <div className={`w-12 h-12 flex items-center justify-center rounded-full text-xl shadow-md border-2 cursor-pointer transition-transform duration-300 ease-out ${dev.isYou ? 'bg-indigo-50 border-indigo-500 scale-110' : 'bg-white border-gray-200 hover:border-gray-400 hover:scale-110'}`}>
              {dev.avatar}
            </div>
            
            {/* The Hover Info Box */}
            <div className={`mt-3 px-4 py-3 bg-white rounded-xl shadow-xl border border-gray-100 opacity-0 group-hover:opacity-100 transition-opacity absolute top-12 w-56 text-left ${dev.isYou ? 'hidden' : ''}`}>
              ...
              <div className="flex gap-3 pt-2 border-t border-gray-100">
                <span className="text-xs font-semibold text-blue-500 hover:text-blue-600 cursor-pointer flex items-center gap-1">𝕏 @{dev.name.split(' ')[0].toLowerCase()}</span>
                <span className="text-xs font-semibold text-gray-600 hover:text-gray-900 cursor-pointer flex items-center gap-1">GH github</span>
              </div>
            </div>

            {/* "You" Indicator */}
            {dev.isYou && (
              <div className="mt-2 font-bold text-xs text-indigo-600 tracking-wide uppercase bg-indigo-50 px-2 py-1 rounded-full border border-indigo-100">You (Stuck)</div>
            )}
          </div>
        ))}
      </div>
